...
BOOST_STATIC_ASSERT(sizeof(LONG) == sizeof(wjl_boost::int32_t));
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
      ...
      while(0 != InterlockedCompareExchange(reinterpret_cast<void**>((wjl_boost::uint_least16_t*)&(m_mutex.m_mutex)), (void*)1, 0))
#else
...
wjl_boost::recursive_mutex* static_mutex::m_pmutex = 0;
wjl_boost::once_flag static_mutex::m_once = BOOST_ONCE_INIT;

extern "C" BOOST_REGEX_DECL void wjl_boost_regex_free_static_mutex()
{
...
{
   m_pmutex = new wjl_boost::recursive_mutex();
   int r = atexit(wjl_boost_regex_free_static_mutex);
   BOOST_ASSERT(0 == r);
}
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
       wjl_boost::call_once(static_mutex::m_once,&static_mutex::init);
      if(0 == m_plock)
         m_plock = new wjl_boost::unique_lock<wjl_boost::recursive_mutex>(*static_mutex::m_pmutex, wjl_boost::defer_lock);
      m_plock->lock();
      m_have_lock = true;
   }
}
