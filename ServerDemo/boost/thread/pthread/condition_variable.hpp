...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost
{
    ...
    inline void condition_variable::wait(unique_lock<mutex>& m)
    {
        ...
        if(! m.owns_lock())
        {
            wjl_boost::throw_exception(condition_error(-1, "wjl_boost::condition_variable::wait() failed precondition mutex not owned"));
        }
#endif
        int res=0;
        {
        ...
        if(res && res != EINTR)
        {
            wjl_boost::throw_exception(condition_error(res, "wjl_boost::condition_variable::wait failed in pthread_cond_wait"));
        }
    }
                ...
                detail::internal_platform_timepoint const &timeout)
    {
        ...
        if (!m.owns_lock())
        {
            wjl_boost::throw_exception(condition_error(EPERM, "wjl_boost::condition_variable::do_wait_until() failed precondition mutex not owned"));
        }
#endif
        int cond_res;
        {
        ...
        if(cond_res)
        {
            wjl_boost::throw_exception(condition_error(cond_res, "wjl_boost::condition_variable::do_wait_until failed in pthread_cond_timedwait"));
        }
        return true;
    }
    ...
    {
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        wjl_boost::pthread::pthread_mutex_scoped_lock internal_lock(&internal_mutex);
#endif
    ...
    public:
        ...
        condition_variable_any()
        {
            ...
            if(res)
            {
                wjl_boost::throw_exception(thread_resource_error(res, "wjl_boost::condition_variable_any::condition_variable_any() failed in pthread_mutex_init"));
            }
            int const res2 = pthread::cond_init(cond);
            if(res2)
            {
                BOOST_VERIFY(!pthread_mutex_destroy(&internal_mutex));
                wjl_boost::throw_exception(thread_resource_error(res2, "wjl_boost::condition_variable_any::condition_variable_any() failed in pthread::cond_init"));
            }
        }
        ~condition_variable_any()
        {
        ...
        void wait(lock_type& m)
        {
            int res=0;
            {
                ...
                detail::interruption_checker check_for_interruption(&internal_mutex,&cond);
#else
                wjl_boost::pthread::pthread_mutex_scoped_lock check_for_interruption(&internal_mutex);
#endif
                guard.activate(m);
            ...
            if(res)
            {
                wjl_boost::throw_exception(condition_error(res, "wjl_boost::condition_variable_any::wait() failed in pthread_cond_wait"));
            }
        }
        ...
        template<typename lock_type>
        bool timed_wait(lock_type& m,wjl_boost::system_time const& abs_time)
        {
        ...
        template<typename lock_type,typename predicate_type>
        bool timed_wait(lock_type& m,wjl_boost::system_time const& abs_time, predicate_type pred)
        {
#if defined BOOST_THREAD_WAIT_BUG
            const detail::real_platform_timepoint ts(abs_time + BOOST_THREAD_WAIT_BUG);
#else
        ...
        {
            const wjl_boost::detail::internal_platform_timepoint ts(t);
        ...
        wait_for(
                ...
                Predicate pred)
        {
            return wait_until(lock, chrono::steady_clock::now() + d, wjl_boost::move(pred));
        }
#endif
    ...
    private:
          ...
          detail::internal_platform_timepoint const &timeout)
        {
          ...
          if(res)
          {
              wjl_boost::throw_exception(condition_error(res, "wjl_boost::condition_variable_any::do_wait_until() failed in pthread_cond_timedwait"));
          }
          return true;
        }
    };
