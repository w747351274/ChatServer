...
{
//typedef wjl_boost::int_least64_t time_max_t;
typedef wjl_boost::intmax_t time_max_t;
...
inline FP init_steady_clock(kern_return_t & err)
{
  ...
  namespace detail
  {
    ...
    inline timespec ns_to_timespec(wjl_boost::time_max_t const& ns)
    {
      wjl_boost::time_max_t s = ns / 1000000000l;
      timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline wjl_boost::time_max_t timespec_to_ns(timespec const& ts)
    {
      return static_cast<wjl_boost::time_max_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
#endif
    ...
    struct platform_duration
    {
      ...
      explicit platform_duration(wjl_boost::time_max_t const& ns = 0) : ts_val(ns_to_timespec(ns)) {}
      wjl_boost::time_max_t getNs() const { return timespec_to_ns(ts_val); }
#else
      explicit platform_duration(wjl_boost::time_max_t const& ns = 0) : ns_val(ns) {}
      wjl_boost::time_max_t getNs() const { return ns_val; }
#endif

#if defined BOOST_THREAD_USES_DATETIME
      platform_duration(wjl_boost::posix_time::time_duration const& rel_time)
      {
#if defined BOOST_THREAD_CHRONO_POSIX_API || defined BOOST_THREAD_CHRONO_MAC_API
...
#else
        ns_val = static_cast<wjl_boost::time_max_t>(rel_time.total_seconds()) * 1000000000l;
      ...
      wjl_boost::time_max_t getMs() const
      {
        const wjl_boost::time_max_t ns = getNs();
    ...
    struct real_platform_timepoint
    {
      ...
      explicit real_platform_timepoint(wjl_boost::time_max_t const& ns) : dur(ns) {}
      wjl_boost::time_max_t getNs() const { return dur.getNs(); }

#if defined BOOST_THREAD_USES_DATETIME
      real_platform_timepoint(wjl_boost::system_time const& abs_time)
        : dur(abs_time - wjl_boost::posix_time::from_time_t(0)) {}
#endif
    ...
    struct real_platform_clock
    {
      ...
      {
#if defined(BOOST_THREAD_CHRONO_WINDOWS_API)
        wjl_boost::detail::winapi::FILETIME_ ft;
        wjl_boost::detail::winapi::GetSystemTimeAsFileTime(&ft);  // never fails
        wjl_boost::time_max_t ns = ((((static_cast<wjl_boost::time_max_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime) - 116444736000000000LL) * 100LL);
  ...
  struct mono_platform_timepoint
  {
    ...
    explicit mono_platform_timepoint(wjl_boost::time_max_t const& ns) : dur(ns) {}
    wjl_boost::time_max_t getNs() const { return dur.getNs(); }
...
#else
      wjl_boost::time_max_t ns = (std::numeric_limits<wjl_boost::time_max_t>::max)();
  ...
  struct mono_platform_clock
  {
    static mono_platform_timepoint now()
    {
      ...
      wjl_boost::detail::winapi::LARGE_INTEGER_ freq;
      if ( !wjl_boost::detail::winapi::QueryPerformanceFrequency( &freq ) )
      {
      ...
      wjl_boost::detail::winapi::LARGE_INTEGER_ pcount;
      unsigned times=0;
      while ( ! wjl_boost::detail::winapi::QueryPerformanceCounter( &pcount ) )
      {
        if ( ++times > 3 )
        {
      ...
      return mono_platform_timepoint(static_cast<wjl_boost::time_max_t>(ns));
#else
