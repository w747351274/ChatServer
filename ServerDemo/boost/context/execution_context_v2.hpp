...
#if defined(BOOST_NO_CXX17_STD_APPLY)
        Ctx cc = wjl_boost::context::detail::apply( std::move( fn_), std::move( tpl) );
#else
...
transfer_t ecv2_context_ontop( transfer_t t) {
    ...
    try {
        // execute function
#if defined(BOOST_NO_CXX17_STD_APPLY)
        std::get< 1 >( std::get< 1 >( * p) ) = helper< sizeof ... (Args) >::convert( wjl_boost::context::detail::apply( fn, std::move( args) ) );
#else
