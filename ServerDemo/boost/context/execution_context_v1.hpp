...
struct BOOST_CONTEXT_DECL ecv1_activation_record {
    typedef wjl_boost::intrusive_ptr< ecv1_activation_record >    ptr_t;
    ...
    ecv1_activation_record( fcontext_t fctx_, stack_context sctx_) noexcept :
        fctx{ fctx_ },
        sctx( sctx_ ), // sctx{ sctx_ } - clang-3.6: no viable conversion from 'wjl_boost::context::stack_context' to 'std::size_t'
        main_ctx{ false } {
    } 
...
#if defined(BOOST_NO_CXX17_STD_APPLY)
    dp->data = wjl_boost::context::detail::apply( fn, std::tie( data) );
#else
...
#if defined(BOOST_NO_CXX17_STD_APPLY)
        wjl_boost::context::detail::apply( fn_, std::tuple_cat( args_, std::tie( data) ) );
#else
...
private:
    ...
    typedef wjl_boost::intrusive_ptr< detail::ecv1_activation_record >    ptr_t;
