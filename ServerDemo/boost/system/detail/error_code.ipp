...
# define BOOST_SYSTEM_DECL_ BOOST_SYSTEM_DECL
  ...
  {
    using namespace wjl_boost::system::errc;
#if defined(__PGI)
      using wjl_boost::system::errc::invalid_argument;
#endif
...
# if defined(BOOST_WINDOWS_API)
    ...
    using namespace wjl_boost::winapi; // for error codes
...
# else
  ...
  BOOST_SYSTEM_DECL_ std::string system_error_category::message( int ev ) const
  {
    ...
    for (;;)
    {
        wjl_boost::winapi::DWORD_ retval = wjl_boost::winapi::FormatMessageW(
            wjl_boost::winapi::FORMAT_MESSAGE_FROM_SYSTEM_ |
            wjl_boost::winapi::FORMAT_MESSAGE_IGNORE_INSERTS_,
            NULL,
            ev,
            wjl_boost::winapi::MAKELANGID_(wjl_boost::winapi::LANG_NEUTRAL_,
            wjl_boost::winapi::SUBLANG_DEFAULT_), // Default language
            &buf[0],
            static_cast<wjl_boost::winapi::DWORD_>(buf.size()),
            NULL
        );
        ...
        else if (wjl_boost::winapi::GetLastError() !=
          wjl_boost::winapi::ERROR_INSUFFICIENT_BUFFER_)
        {
    ...
    wjl_boost::winapi::LPSTR_ narrow_buffer =
#if defined(__GNUC__)
      (wjl_boost::winapi::LPSTR_)__builtin_alloca(num_chars);
#else
      (wjl_boost::winapi::LPSTR_)_alloca(num_chars);
#endif

    if (wjl_boost::winapi::WideCharToMultiByte(wjl_boost::winapi::CP_ACP_, 0,
...
#else
    wjl_boost::winapi::LPVOID_ lpMsgBuf = 0;
    wjl_boost::winapi::DWORD_ retval = wjl_boost::winapi::FormatMessageA(
        wjl_boost::winapi::FORMAT_MESSAGE_ALLOCATE_BUFFER_ |
        wjl_boost::winapi::FORMAT_MESSAGE_FROM_SYSTEM_ |
        wjl_boost::winapi::FORMAT_MESSAGE_IGNORE_INSERTS_,
        NULL,
        ev,
        wjl_boost::winapi::MAKELANGID_(wjl_boost::winapi::LANG_NEUTRAL_,
        wjl_boost::winapi::SUBLANG_DEFAULT_), // Default language
        (wjl_boost::winapi::LPSTR_) &lpMsgBuf,
        0,
        NULL
    );
    ...
    std::string str(static_cast<wjl_boost::winapi::LPCSTR_>(lpMsgBuf));
# endif
    while ( str.size()
      && (str[str.size()-1] == '\n' || str[str.size()-1] == '\r') )
