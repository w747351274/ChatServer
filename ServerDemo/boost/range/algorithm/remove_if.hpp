...
inline BOOST_DEDUCED_TYPENAME wjl_boost::range_iterator<ForwardRange>::type
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::remove_if(wjl_boost::begin(rng), wjl_boost::end(rng), pred);
}

/// \overload
template< class ForwardRange, class UnaryPredicate >
inline BOOST_DEDUCED_TYPENAME wjl_boost::range_iterator<const ForwardRange>::type
...
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    ...
    return range_return<ForwardRange,re>::pack(
        std::remove_if(wjl_boost::begin(rng), wjl_boost::end(rng), pred),
        rng);
}
