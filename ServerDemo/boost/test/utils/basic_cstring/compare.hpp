...
template<class CharT>
inline bool
operator <( wjl_boost::unit_test::basic_cstring<CharT> const& x,
            wjl_boost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename wjl_boost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size()
            ? x.size() < y.size()
