...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(wjl_boost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(wjl_boost::declval<U&>())); // does not work with wjl_boost::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(wjl_boost::declval<U&>())); // does not work with wjl_boost::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(wjl_boost::declval<
        typename wjl_boost::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename wjl_boost::add_const<T>::type T_const;
    typedef typename wjl_boost::add_pointer<T_const>::type const_iterator;
