...
// forward declaration
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { template<class T> class optional; }

namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const wjl_boost::any& value_store) const
    {
        const T* value = wjl_boost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                wjl_boost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                wjl_boost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const wjl_boost::any& value);
    }
    ...
    */
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            wjl_boost::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(wjl_boost::any& v, 
    ...
    {
        if (v.empty()) {
            v = wjl_boost::any(std::vector<T>());
        }
        std::vector<T>* tv = wjl_boost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(wjl_boost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                wjl_boost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
                  ...
                  int)
    {
        ...
        wjl_boost::any a;
        validate(a, s, (T*)0, 0);
        v = wjl_boost::any(wjl_boost::optional<T>(wjl_boost::any_cast<T>(a)));
    }

    template<class T, class charT>
    void 
    typed_value<T, charT>::
    xparse(wjl_boost::any& value_store, 
    ...
    value()
    {
        ...
        return wjl_boost::program_options::value<T>(0);
    }
