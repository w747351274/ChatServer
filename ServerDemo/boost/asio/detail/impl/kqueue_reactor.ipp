...
kqueue_reactor::kqueue_reactor(wjl_boost::asio::execution_context& ctx)
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  {
    wjl_boost::system::error_code error(errno,
        wjl_boost::asio::error::get_system_category());
    wjl_boost::asio::detail::throw_error(error);
  }
}
...
void kqueue_reactor::notify_fork(
    wjl_boost::asio::execution_context::fork_event fork_ev)
{
  if (fork_ev == wjl_boost::asio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      wjl_boost::system::error_code ec(errno,
          wjl_boost::asio::error::get_system_category());
      wjl_boost::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          wjl_boost::system::error_code ec(errno,
              wjl_boost::asio::error::get_system_category());
          wjl_boost::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        {
          op->ec_ = wjl_boost::system::error_code(errno,
              wjl_boost::asio::error::get_system_category());
          scheduler_.post_immediate_completion(op, is_continuation);
          return;
        }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    wjl_boost::system::error_code ec(errno,
        wjl_boost::asio::error::get_system_category());
    wjl_boost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
