...
-> decltype(wjl_boost::pointer_traits<T>::to_address(v))
{
    return wjl_boost::pointer_traits<T>::to_address(v);
}
...
{
    return wjl_boost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return wjl_boost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return wjl_boost::to_address(v.operator->());
}
#endif
