...
public:
    ...
    explicit newline_filter(int target) : flags_(target)
    {
        ...
        {
            wjl_boost::throw_exception(std::logic_error("bad flags"));
        }
    }
...
private:
    ...
    bool newline(Sink& dest)
    {
        ...
        switch (flags_ & iostreams::newline::platform_mask) {
        case iostreams::newline::posix:
            success = wjl_boost::iostreams::put(dest, LF);
            break;
        case iostreams::newline::mac:
            success = wjl_boost::iostreams::put(dest, CR);
            break;
        case iostreams::newline::dos:
            if ((flags_ & f_has_LF) != 0) {
                if ((success = wjl_boost::iostreams::put(dest, LF)))
                    flags_ &= ~f_has_LF;
            } else if (wjl_boost::iostreams::put(dest, CR)) {
                if (!(success = wjl_boost::iostreams::put(dest, LF)))
                    flags_ |= f_has_LF;
            }
            break;
...
private:
    void fail() { wjl_boost::throw_exception(newline_error(source())); }
