...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(BOOST_HAS_THREADS)
      wjl_boost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
#if defined(BOOST_HAS_THREADS)
  wjl_boost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(BOOST_HAS_THREADS)
    wjl_boost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
private:
  friend class wjl_boost::multi_index::detail::iter_adaptor_access;
...
template<typename Iterator,typename Container>
struct version<
  wjl_boost::multi_index::safe_mode::safe_iterator<Iterator,Container>
>
{
  BOOST_STATIC_CONSTANT(
    int,value=wjl_boost::serialization::version<Iterator>::value);
};
} /* namespace serialization */
#endif
