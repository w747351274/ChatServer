...
namespace detail
{
    ...
    public:
        ...
        typedef wjl_boost::reverse_iterator<iterator>        reverse_iterator;
        typedef wjl_boost::reverse_iterator<const_iterator>  const_reverse_iterator;
        typedef typename wjl_boost::mpl::if_c< wjl_boost::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, wjl_boost::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const wjl_boost::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const wjl_boost::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, wjl_boost::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const wjl_boost::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const wjl_boost::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const wjl_boost::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const wjl_boost::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const wjl_boost::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const wjl_boost::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, wjl_boost::has_trivial_destructor<T>() );
        }
        ...
        void auto_buffer_destroy()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( wjl_boost::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const wjl_boost::false_type& )
        {
            BOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type, const wjl_boost::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, wjl_boost::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const wjl_boost::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const wjl_boost::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const wjl_boost::false_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
            wjl_boost::multi_index::detail::scope_guard guard =
                wjl_boost::multi_index::detail::make_obj_guard( *this,
        ...
        pointer move_to_new_buffer( size_type new_capacity, const wjl_boost::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
                                 ...
                                 const wjl_boost::true_type& )
        {
            ...
            wjl_boost::swap( l.size_, r.size_ );
            wjl_boost::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const wjl_boost::false_type& )
        {
            BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                wjl_boost::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const wjl_boost::true_type& )
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const wjl_boost::false_type& )
        {
        ...
        {
            grow_back( n, wjl_boost::has_trivial_constructor<T>() );
        }

        void grow_back_one( const wjl_boost::true_type& )
        {
            BOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const wjl_boost::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( wjl_boost::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, wjl_boost::has_trivial_assign<T>() );
        }
        ...
        {
            BOOST_ASSERT( !empty() );
            auto_buffer_destroy( buffer_ + size_ - 1, wjl_boost::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                wjl_boost::swap( get_allocator(), r.get_allocator() );
                wjl_boost::swap( members_.capacity_, r.members_.capacity_ );
                wjl_boost::swap( buffer_, r.buffer_ );
                wjl_boost::swap( size_, r.size_ );
                BOOST_ASSERT( is_valid() );
            ...
            if( exactly_one_on_stack )
            {
                ...
                wjl_boost::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                wjl_boost::swap( members_.capacity_, r.members_.capacity_ );
                wjl_boost::swap( size_, r.size_ );
            ...
            BOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, wjl_boost::has_trivial_assign<T>() );
            BOOST_ASSERT( is_valid() );
            BOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef wjl_boost::aligned_storage< N * sizeof(T),
                                        wjl_boost::alignment_of<T>::value >
                               storage;
