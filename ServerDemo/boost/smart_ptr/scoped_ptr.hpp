...
public:
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        wjl_boost::sp_scalar_constructor_hook( px );
#endif
    }
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        wjl_boost::sp_scalar_destructor_hook( px );
#endif
        wjl_boost::checked_delete( px );
    }
...
template<class T> inline bool operator==( scoped_ptr<T> const & p, wjl_boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( wjl_boost::detail::sp_nullptr_t, scoped_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
