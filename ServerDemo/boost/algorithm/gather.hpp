...
#include <boost/bind.hpp>           // for wjl_boost::bind
#include <boost/range/begin.hpp>    // for wjl_boost::begin(range)
#include <boost/range/end.hpp>      // for wjl_boost::end(range)
...
//  while the second call partitions the rest of the sequence.
    return std::make_pair (
        std::stable_partition ( first, pivot, !wjl_boost::bind<bool> ( pred, _1 )),
        std::stable_partition ( pivot, last,   wjl_boost::bind<bool> ( pred, _1 )));
}
...
template <
    ...
    typename Pred>                  // Pred models UnaryPredicate
std::pair<
    typename wjl_boost::range_iterator<const BidirectionalRange>::type,
    typename wjl_boost::range_iterator<const BidirectionalRange>::type>
gather (
    const BidirectionalRange &range,
    typename wjl_boost::range_iterator<const BidirectionalRange>::type pivot,
    Pred pred )
{
    return wjl_boost::algorithm::gather ( wjl_boost::begin ( range ), wjl_boost::end ( range ), pivot, pred );
}
