    ...
    collection_size_type count)
{
    ...
    #if 0
        wjl_boost::archive::library_version_type library_version(
            ar.get_library_version()
        );
        if(wjl_boost::archive::library_version_type(3) < library_version){
    ...
    while(count-- > 0){
        ...
        wjl_boost::serialization::save_construct_data_adl(
            ar, 
            wjl_boost::addressof(*it),
            item_version
        );
        ar << wjl_boost::serialization::make_nvp("item", *it++);
    }
}
