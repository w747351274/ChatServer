...
{
    return ::wjl_boost::forward<Visitable>(visitable).apply_visitor(visitor);
}
#else
template <typename Visitor, typename Visitable>
inline
...
struct result_multideduce1 {
    ...
    typedef typename wjl_boost::mpl::begin<types>::type begin_it;
    typedef typename wjl_boost::mpl::advance<
        begin_it, wjl_boost::mpl::int_<wjl_boost::mpl::size<types>::type::value - 1>
    >::type                                         last_it;
    ...
    static Visitor& vis() BOOST_NOEXCEPT {
        ...
        // Because of that, we can not use `wjl_boost::decval<Visitor&>()` here.
        Visitor&(*f)() = 0; // pointer to function
        return f();
    }

    static decltype(auto) deduce_impl(last_it, unsigned /*helper*/) {
        typedef typename wjl_boost::mpl::deref<last_it>::type value_t;
        return vis()( wjl_boost::declval< value_t& >() );
    }

    template <class It>
    static decltype(auto) deduce_impl(It, unsigned helper) {
        typedef typename wjl_boost::mpl::next<It>::type next_t;
        typedef typename wjl_boost::mpl::deref<It>::type value_t;
        if (helper == wjl_boost::mpl::distance<begin_it, It>::type::value) {
...
struct result_wrapper1
{
    ...
    explicit result_wrapper1(Visitor&& visitor) BOOST_NOEXCEPT
        : visitor_(::wjl_boost::forward<Visitor>(visitor))
    {}

    template <class T>
    result_type operator()(T&& val) const {
        return visitor_(::wjl_boost::forward<T>(val));
    }
};
...
inline decltype(auto) apply_visitor(Visitor&& visitor, Visitable&& visitable,
    typename wjl_boost::disable_if<
        wjl_boost::detail::variant::has_result_type<Visitor>
    >::type* = 0)
{
    wjl_boost::detail::variant::result_wrapper1<Visitor, typename remove_reference<Visitable>::type> cpp14_vis(::wjl_boost::forward<Visitor>(visitor));
    return ::wjl_boost::forward<Visitable>(visitable).apply_visitor(cpp14_vis);
}
