...
BOOST_STATIC_ASSERT((AdaptiveSortInsertionSortThreshold&(AdaptiveSortInsertionSortThreshold-1)) == 0);
   ...
   typedef ::wjl_boost::uintptr_t uintptr_t;
#else
...
class adaptive_xbuf
{
   ...
   {
      if(n <= m_size){
         wjl_boost::move(first, first+n, m_ptr);
         ...
         RandRawIt result = wjl_boost::move(first, first+m_size, m_ptr);
         wjl_boost::uninitialized_move(first+m_size, first+n, result);
         m_size = n;
      }
   }
   ...
   void push_back(RandIt first, std::size_t n)
   {
      ...
      wjl_boost::uninitialized_move(first, first+n, m_ptr+m_size);
      m_size += n;
   }
   ...
   void insert(iterator pos, RandIt it)
   {
      if(pos == (m_ptr + m_size)){
         ...
         //m_size updated
         wjl_boost::move_backward(pos, m_ptr+m_size-2, m_ptr+m_size-1);
         *pos = wjl_boost::move(*it);
      }
   }
   ...
   void initialize_until(std::size_t const size, T &t)
   {
      BOOST_ASSERT(m_size < m_capacity);
      if(m_size < size){
         ::new((void*)&m_ptr[m_size]) T(::wjl_boost::move(t));
         ++m_size;
         for(; m_size != size; ++m_size){
            ::new((void*)&m_ptr[m_size]) T(::wjl_boost::move(m_ptr[m_size-1]));
         }
         t = ::wjl_boost::move(m_ptr[m_size-1]);
      }
   }
...
{
   if(begin != with){
      ::wjl_boost::adl_move_swap_ranges(begin, end, with);
      ::wjl_boost::adl_move_swap(*key_next, *key_range2);
      if(key_next == key_mid){
         key_mid = key_range2;
      }
   ...
   (RandIt first1, RandIt last1, RandIt const last2, bool *const pis_range1_A, Compare comp)
{
   ...
   if(first1 != last1 && comp(*last1, last1[-1])){
      do{
         ...
         last1  = wjl_boost::movelib::lower_bound(last1, last2, *first1, comp);
   ...
   , Compare comp)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(wjl_boost::movelib::is_sorted_and_unique(key_first, key_first + key_count, key_comp));
      ...
      , Buf &xbuf)
{
   if(first != middle && middle != last && comp(*middle, middle[-1])){
      ...
      if(len1 <= len2){
         first = wjl_boost::movelib::upper_bound(first, middle, *middle, comp);
         xbuf.move_assign(first, size_type(middle-first));
         op_merge_with_right_placed
            (xbuf.data(), xbuf.end(), first, middle, last, comp, op);
      }
      else{
         last = wjl_boost::movelib::lower_bound(middle, last, middle[-1], comp);
   ...
   if(max_collected){
      ...
      if(xbuf.capacity() >= max_collected){
         ...
         while(u != last && h < max_collected){
            typename XBuf::iterator const r = wjl_boost::movelib::lower_bound(ph0, xbuf.end(), *u, comp);
            //If key not found add it to [h, h+h0)
            if(r == xbuf.end() || comp(*u, *r) ){
               RandIt const new_h0 = wjl_boost::move(search_end, u, h0);
               search_end = u;
               ++search_end;
               ++h;
               xbuf.insert(r, u);
         ...
         wjl_boost::move_backward(first, h0, h0+h);
         wjl_boost::move(xbuf.data(), xbuf.end(), first);
      }
      else{
         while(u != last && h < max_collected){
            RandIt const r = wjl_boost::movelib::lower_bound(h0, search_end, *u, comp);
            //If key not found add it to [h, h+h0)
...
{
   wjl_boost::movelib::inplace_stable_sort(first, last, comp);
}
                    ...
                    , XBuf & xbuf)
{
   ...
   BOOST_ASSERT(wjl_boost::movelib::is_sorted_and_unique(first, last, comp));
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left, ++key_first, min_check -= min_check != 0, max_check -= max_check != 0){
      ...
      if(first_reg == dest){
         dest = next_key_idx ? ::wjl_boost::adl_move_swap_ranges(first_min, last_min, first_reg)
                             : last_reg;
      }
      else{
