...
namespace date_time {
  ...
  private:
    static time_type create_time(time_converter converter)
    {
      ...
      wjl_boost::uint32_t sub_sec = tv.tv_usec;
#elif defined(BOOST_HAS_FTIME)
      wjl_boost::winapi::FILETIME_ ft;
      wjl_boost::winapi::GetSystemTimeAsFileTime(&ft);
#if BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3205))
      // Some runtime library implementations expect local times as the norm for ctime functions.
      {
        wjl_boost::winapi::FILETIME_ local_ft;
        wjl_boost::winapi::FileTimeToLocalFileTime(&ft, &local_ft);
        ft = local_ft;
      }
#endif

      wjl_boost::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
                                                              // and cannot be before 1970-Jan-01
      std::time_t t = static_cast<std::time_t>(micros / 1000000UL); // seconds since epoch
      // microseconds -- static casts suppress warnings
      wjl_boost::uint32_t sub_sec = static_cast<wjl_boost::uint32_t>(micros % 1000000UL);
#else
#error Internal Boost.DateTime error: BOOST_DATE_TIME_HAS_HIGH_PRECISION_CLOCK is defined, however neither gettimeofday nor FILETIME support is detected.
     ...
     */
    static wjl_boost::uint64_t file_time_to_microseconds(wjl_boost::winapi::FILETIME_ const& ft)
    {
      // shift is difference between 1970-Jan-01 & 1601-Jan-01
      // in 100-nanosecond units
      const wjl_boost::uint64_t shift = 116444736000000000ULL; // (27111902 << 32) + 3577643008

      // 100-nanos since 1601-Jan-01
      wjl_boost::uint64_t ft_as_integer = (static_cast< wjl_boost::uint64_t >(ft.dwHighDateTime) << 32) | static_cast< wjl_boost::uint64_t >(ft.dwLowDateTime);
