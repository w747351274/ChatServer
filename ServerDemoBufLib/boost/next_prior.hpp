...
//    const std::list<T>::iterator prev = wjl_boost::prior(p);
//    const std::list<T>::iterator next = wjl_boost::next(prev, 2);
...
struct next_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        wjl_boost::iterators::advance(x, n);
        return x;
    }
};
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        wjl_boost::iterators::reverse_iterator< T > rx(x);
        wjl_boost::iterators::advance(rx, n);
        return rx.base();
    }
};
