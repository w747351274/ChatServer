...
namespace date_time {
  ...
  */
  ...
  class BOOST_SYMBOL_VISIBLE time_duration : private
      wjl_boost::less_than_comparable<T
    , wjl_boost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, wjl_boost::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(wjl_boost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    // The argument (ss) must be an integral type
    template <typename T>
    explicit subsecond_duration(T const& ss,
                                typename wjl_boost::enable_if<wjl_boost::is_integral<T>, void>::type* = 0) :
