...
RandIt rotate_gcd(RandIt first, RandIt middle, RandIt last)
{
   ...
   if (middle == ret){
      wjl_boost::adl_move_swap_ranges(first, middle, middle);
   }
   else{
...
void merge_bufferless_ON2(RandIt first, RandIt middle, RandIt last, Compare comp)
{
   if((middle - first) < (last - middle)){
      while(first != middle){
         ...
         middle = wjl_boost::movelib::lower_bound(middle, last, *first, comp);
      ...
      while(middle != last){
         RandIt p = wjl_boost::movelib::upper_bound(first, middle, last[-1], comp);
   ...
   (RandIt first, RandIt middle, RandIt last, Distance len1, Distance len2, Compare comp)
{
   ...
   while(1) {
      ...
      if (len1 > len2) {
         len11 = len1 / 2;
         first_cut +=  len11;
         second_cut = wjl_boost::movelib::lower_bound(middle, last, *first_cut, comp);
         len22 = size_type(second_cut - middle);
      }
      else {
         len22 = len2 / 2;
         second_cut += len22;
         first_cut = wjl_boost::movelib::upper_bound(first, middle, *second_cut, comp);
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new((iterator_to_raw_pointer)(dest_first)) value_type(::wjl_boost::move(*first));
            d.incr();
         }
         d.release();
         InputOutIterator end = ::wjl_boost::move(first, last, original_r_first);
         BOOST_ASSERT(end == r_last);
         (void)end;
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new((iterator_to_raw_pointer)(dest_first)) value_type(::wjl_boost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
   ...
   , Compare comp)
{
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new(&*dest_first) value_type(::wjl_boost::move(*first));
            d.incr();
         }
         d.release();
         BidirOutIterator end = ::wjl_boost::move(first, last, original_r_first);
         BOOST_ASSERT(end == r_last);
         (void)end;
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new(&*dest_first) value_type(::wjl_boost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
