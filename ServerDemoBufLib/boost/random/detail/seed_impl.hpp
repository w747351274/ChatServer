...
struct seed_type
{
    typedef typename wjl_boost::mpl::if_<wjl_boost::is_integral<T>,
        T,
        wjl_boost::uint32_t
    >::type type;
};
...
void generate_from_real(Engine& eng, Iter begin, Iter end)
{
    ...
    wjl_boost::uint_least32_t saved_bits = 0;
    ...
    while(true) {
        ...
        if(Bits < 32 && available_bits < 32 - remaining_bits) {
            saved_bits |= wjl_boost::uint_least32_t(val) << remaining_bits;
            remaining_bits += Bits;
        } else {
            ...
            if(Bits < 32 || remaining_bits != 0) {
                wjl_boost::uint_least32_t divisor =
                    (wjl_boost::uint_least32_t(1) << (32 - remaining_bits));
                wjl_boost::uint_least32_t extra_bits = wjl_boost::uint_least32_t(fmod(val, mult32)) & (divisor - 1);
                val = val / divisor;
            ...
            if(Bits >= 32) {
                for(; available_bits >= 32; available_bits -= 32) {
                    wjl_boost::uint_least32_t word = wjl_boost::uint_least32_t(fmod(val, mult32));
                    val /= mult32;
                    *begin++ = word;
                    if(begin == end) return;
                }
            ...
            saved_bits = static_cast<wjl_boost::uint_least32_t>(val);
        }
    }
}
...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    typedef typename wjl_boost::random::traits::make_unsigned<IntType>::type unsigned_type;
    int remaining_bits = 0;
    wjl_boost::uint_least32_t saved_bits = 0;
    unsigned_type range = wjl_boost::random::detail::subtract<IntType>()((eng.max)(), (eng.min)());
    ...
    while(true) {
        unsigned_type val;
        do {
            val = wjl_boost::random::detail::subtract<IntType>()(eng(), (eng.min)());
        } while(limit != range && val > limit);
        val &= mask;
        int available_bits = bits;
        if(available_bits == 32) {
            *begin++ = static_cast<wjl_boost::uint_least32_t>(val) & 0xFFFFFFFFu;
            if(begin == end) return;
        } else if(available_bits % 32 == 0) {
            for(int i = 0; i < available_bits / 32; ++i) {
                wjl_boost::uint_least32_t word = wjl_boost::uint_least32_t(val) & 0xFFFFFFFFu;
            ...
            if(bits < 32 || remaining_bits != 0) {
                wjl_boost::uint_least32_t extra_bits = wjl_boost::uint_least32_t(val) & ((wjl_boost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
...
template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, wjl_boost::mpl::true_)
{
    return detail::generate_from_int(eng, first, last);
}

template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, wjl_boost::mpl::false_)
{
...
{
    return detail::generate_impl(eng, first, last, wjl_boost::random::traits::is_integral<typename Engine::result_type>());
}
...
{
    static const int log = ::wjl_boost::mpl::if_c<(m == 0),
        ::wjl_boost::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
        ::wjl_boost::static_log2<m> >::type::value;
    static const int k =
        (log + ((~(static_cast<IntType>(2) << (log - 1)) & m)? 32 : 31)) / 32;
    ::wjl_boost::uint_least32_t array[log / 32 + 4];
...
IntType get_one_int(Iter& first, Iter last)
{
    ...
    for(int j = 0; j < k; ++j) {
        if(first == last) {
            wjl_boost::throw_exception(::std::invalid_argument("Not enough elements in call to seed."));
        }
...
{
    wjl_boost::uint_least32_t storage[((w+31)/32) * n];
    ...
    for(std::size_t j = 0; j < n; j++) {
        ...
        x[j] = val & ::wjl_boost::low_bits_mask_t<w>::sig_bits;
    }
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], wjl_boost::mpl::true_)
{
    BOOST_STATIC_ASSERT_MSG(wjl_boost::is_integral<IntType>::value, "Sorry but this routine has not been ported to non built-in integers as it relies on a reinterpret_cast.");
    typedef typename wjl_boost::make_unsigned<IntType>::type unsigned_array[n];
    seed_array_int_impl<w>(seq, reinterpret_cast<unsigned_array&>(x));
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], wjl_boost::mpl::false_)
{
...
{
    seed_array_int_impl<w>(seq, x, wjl_boost::random::traits::is_signed<IntType>());
}
...
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], wjl_boost::mpl::true_)
{
    BOOST_STATIC_ASSERT_MSG(wjl_boost::is_integral<IntType>::value, "Sorry but this routine has not been ported to non built-in integers as it relies on a reinterpret_cast.");
    typedef typename wjl_boost::make_unsigned<IntType>::type unsigned_array[n];
...
template<int w, std::size_t n, class Iter, class IntType>
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], wjl_boost::mpl::false_)
{
...
{
    fill_array_int_impl<w>(first, last, x, wjl_boost::random::traits::is_signed<IntType>());
}

template<int w, std::size_t n, class RealType>
void seed_array_real_impl(const wjl_boost::uint_least32_t* storage, RealType (&x)[n])
{
    wjl_boost::uint_least32_t mask = ~((~wjl_boost::uint_least32_t(0)) << (w%32));
...
void fill_array_real(Iter& first, Iter last, RealType (&x)[n])
{
    ...
    for(j = 0; j < n; ++j) {
        ...
        for(int k = 0; k < w/32; ++k, ++first) {
            if(first == last) wjl_boost::throw_exception(std::invalid_argument("Not enough elements in call to seed."));
            val += *first * mult;
            mult *= two32;
        }
        if(mask != 0) {
