...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::wjl_boost::iostreams::pipeline< \
        ::wjl_boost::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::wjl_boost::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::wjl_boost::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/
