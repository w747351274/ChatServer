...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { namespace iostreams {
                    
//------------------Definitions of constants----------------------------------//

namespace gzip {

using namespace wjl_boost::iostreams::zlib;
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += wjl_boost::iostreams::write_if(snk, header_.data() + offset_, amt);
...
private:
    ...
    template<typename Sink>
    static void write_long(long n, Sink& next, wjl_boost::mpl::true_)
    {
        wjl_boost::iostreams::put(next, static_cast<char>(0xFF & n));
        wjl_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        wjl_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        wjl_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }
    template<typename Sink>
    static void write_long(long, Sink&, wjl_boost::mpl::false_)
    {
    }
    template<typename Sink>
...
} // End namespace wjl_boost::iostreams::detail.
...
public:
        ...
        while(result < n) {
            ...
            if (state_ == s_header) {
                ...
                try {
                    ...
                    wjl_boost::throw_exception(gzip_error(e));
                }
            } else { // state_ == s_footer
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        wjl_boost::throw_exception(gzip_error(gzip::bad_crc));
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = wjl_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    wjl_boost::throw_exception(gzip_error(gzip::bad_header));
                ...
                int c = wjl_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    wjl_boost::throw_exception(gzip_error(gzip::bad_footer));
                } else if (traits_type::would_block(c)) {
                    break;
                }
                footer_.process(c);
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        wjl_boost::throw_exception(gzip_error(gzip::bad_crc));
                    c = wjl_boost::iostreams::get(peek);
                    if (traits_type::is_eof(c)) {
                        state_ = s_done;
                    } else {
                        peek.putback(c);
...
private:
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            std::streamsize amt = 
                wjl_boost::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                wjl_boost::throw_exception(
                    wjl_boost::iostreams::detail::bad_putback());
            }
            return true;
        }
...
{
    wjl_boost::iostreams::back_insert_device<std::string> out(footer_);
