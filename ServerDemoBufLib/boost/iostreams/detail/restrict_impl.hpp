 ...
 * class template wjl_boost::iostreams::restriction. If included with the macro
 * BOOST_IOSTREAMS_RESTRICT defined as an identifier, defines the overloaded
 * function template wjl_boost::iostreams::BOOST_IOSTREAMS_RESTRICT, and object 
 * generator for wjl_boost::iostreams::restriction.
 *
...
public:
        ...
        if (end_ != -1 && pos_ + n >= end_) {
            if(pos_ < end_)
                pos_ += iostreams::write(this->component(),
                    snk, s, end_ - pos_);
            wjl_boost::throw_exception(bad_write());
        }
        std::streamsize result = 
    ...
    std::streampos seek(Device& dev, stream_offset off, BOOST_IOS::seekdir way)
    {
        ...
        if (way == BOOST_IOS::beg) {
            ...
            if (pos_ < beg_)
                wjl_boost::throw_exception(bad_seek());
...
{
    if (len < -1 || off < 0)
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("bad offset"));
