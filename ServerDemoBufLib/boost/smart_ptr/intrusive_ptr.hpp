...
public:
    ...
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename wjl_boost::detail::sp_enable_if_convertible<U,T>::type = wjl_boost::detail::sp_empty() )
...
// Move support
    ...
    intrusive_ptr(intrusive_ptr<U> && rhs, typename wjl_boost::detail::sp_enable_if_convertible<U,T>::type = wjl_boost::detail::sp_empty())
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, wjl_boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( wjl_boost::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( wjl_boost::intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return wjl_boost::hash< T* >()( p.get() );
}
