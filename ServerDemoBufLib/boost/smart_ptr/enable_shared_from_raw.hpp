...
{
template<typename T> wjl_boost::shared_ptr<T> shared_from_raw(T *);
template<typename T> wjl_boost::weak_ptr<T> weak_from_raw(T *);
...
private:
    ...
    template<typename T> friend wjl_boost::shared_ptr<T> shared_from_raw(T *);
    template<typename T> friend wjl_boost::weak_ptr<T> weak_from_raw(T *);
    template< class X, class Y > friend inline void detail::sp_enable_shared_from_this( wjl_boost::shared_ptr<X> * ppx, Y const * py, wjl_boost::enable_shared_from_raw const * pe );
#endif
    ...
    template<class X, class Y> void _internal_accept_owner( shared_ptr<X> * ppx, Y * ) const
    {
        ...
        else if( shared_this_.use_count() != 0 )
        {
            ...
            detail::esft2_deleter_wrapper * pd = wjl_boost::get_deleter<detail::esft2_deleter_wrapper>( shared_this_ );
            BOOST_ASSERT( pd != 0 );
...
template<typename T>
wjl_boost::shared_ptr<T> shared_from_raw(T *p)
{
    BOOST_ASSERT(p != 0);
    return wjl_boost::shared_ptr<T>(p->enable_shared_from_raw::shared_from_this(), p);
}

template<typename T>
wjl_boost::weak_ptr<T> weak_from_raw(T *p)
{
    BOOST_ASSERT(p != 0);
    wjl_boost::weak_ptr<T> result;
