...
{
  if (new_size > sizeof(wjl_boost::asio::detail::sockaddr_un_type))
  {
  ...
  else
  {
    path_length_ = new_size
      - offsetof(wjl_boost::asio::detail::sockaddr_un_type, sun_path);
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    wjl_boost::system::error_code ec(wjl_boost::asio::error::name_too_long);
    wjl_boost::asio::detail::throw_error(ec);
  }

  using namespace std; // For memcpy.
  data_.local = wjl_boost::asio::detail::sockaddr_un_type();
