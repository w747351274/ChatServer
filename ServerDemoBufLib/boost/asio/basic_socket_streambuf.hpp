...
#if defined(BOOST_ASIO_HAS_BOOST_DATE_TIME) \
  && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
    typename Clock = wjl_boost::posix_time::ptime,
...
# endif // !defined(BOOST_ASIO_NO_DEPRECATED)
      ...
      expiry_time_(other.expiry_time_)
  {
    ...
    other.ec_ = wjl_boost::system::error_code();
  ...
  {
    init_buffers();
    ec_ = wjl_boost::system::error_code();
   ...
   * buffer.
   */
  const wjl_boost::system::error_code& error() const
  {
    return ec_;
  }
   ...
   * buffer.
   */
  const wjl_boost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * wjl_boost::asio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    ...
    ec_ = wjl_boost::asio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      {
        ec_ = wjl_boost::asio::error::timed_out;
        return traits_type::eof();
      }
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(wjl_boost::asio::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = wjl_boost::asio::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != wjl_boost::asio::error::would_block
          && ec_ != wjl_boost::asio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = wjl_boost::asio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = wjl_boost::asio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    ec_ = wjl_boost::asio::error::not_found;
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != wjl_boost::asio::error::in_progress
          && ec_ != wjl_boost::asio::error::would_block)
        continue;
      ...
      ec_ = wjl_boost::system::error_code(connect_error,
...
#if defined(BOOST_ASIO_HAS_BOOST_DATE_TIME) \
  && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
    return wjl_boost::posix_time::pos_infin;
#else // defined(BOOST_ASIO_HAS_BOOST_DATE_TIME)
      // && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
  ...
  enum { putback_max = 8 };
  wjl_boost::system::error_code ec_;
