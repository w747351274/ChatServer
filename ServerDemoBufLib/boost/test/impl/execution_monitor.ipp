...
extract( wjl_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = wjl_boost::get_error_info<ErrorInfo>( *ex );
...
static void
report_error( execution_exception::error_code ec, wjl_boost::exception const* be, char const* format, va_list* args )
{
...
static void
report_error( execution_exception::error_code ec, wjl_boost::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return wjl_boost::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    wjl_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         wjl_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        wjl_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( wjl_boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( wjl_boost::function<int ()> const& F )
{
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<wjl_boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
#undef CATCH_AND_REPORT_STD_EXCEPTION
    ...
    catch( wjl_boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
#if defined(BOOST_NO_TYPEID) || defined(BOOST_NO_RTTI)
                              "unknown wjl_boost::exception" ); }
#else
                              typeid(ex).name()          ); }
#endif
...
struct forward {
    explicit    forward( wjl_boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    wjl_boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( wjl_boost::function<void ()> const& F )
{
