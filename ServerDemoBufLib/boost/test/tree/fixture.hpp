...
namespace impl_fixture {
    ...
    struct has_setup {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(wjl_boost::declval<U>().setup())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
    ...
    struct has_teardown {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(wjl_boost::declval<U>().teardown())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
...
public:
    // Constructor
    function_based_fixture( wjl_boost::function<void ()> const& setup_, wjl_boost::function<void ()> const& teardown_ )
    : m_setup( setup_ )
...
private:
    ...
    // Data members
    wjl_boost::function<void ()>    m_setup;
    wjl_boost::function<void ()>    m_teardown;
};
