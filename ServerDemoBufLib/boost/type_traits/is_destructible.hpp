...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(wjl_boost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(detail::is_destructible_imp::test<T>(0)) == sizeof(wjl_boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(wjl_boost::is_complete<T>::value, "Arguments to is_destructible must be complete types");
   };
