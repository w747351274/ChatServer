...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost
{
    namespace detail
    {
        ...
        struct basic_recursive_mutex_impl
        {
            ...
            {
                long const current_thread_id=wjl_boost::winapi::GetCurrentThreadId();
            ...
            bool timed_lock(::wjl_boost::system_time const& target)
            {
                long const current_thread_id=wjl_boost::winapi::GetCurrentThreadId();
            ...
            {
                long const current_thread_id=wjl_boost::detail::winapi::GetCurrentThreadId();
                return try_recursive_lock(current_thread_id) || try_timed_lock(current_thread_id,target);
            }
#endif
        ...
        private:
            ...
            {
                if(::wjl_boost::detail::interlocked_read_acquire(&locking_thread_id)==current_thread_id)
                {
                    ++recursion_count;
                    return true;
                }
...
#if defined BOOST_THREAD_USES_DATETIME
            bool try_timed_lock(long current_thread_id,::wjl_boost::system_time const& target)
            {
