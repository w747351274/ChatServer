...
using wjl_boost::timer::nanosecond_type;
using wjl_boost::timer::cpu_times;
...
namespace
{
  ...
  //  be as low as 10, although will be 15 for many common platforms.
  {
    if (places > 9)
      ...
      places = wjl_boost::timer::default_places;
 
    wjl_boost::io::ios_flags_saver ifs(os);
    wjl_boost::io::ios_precision_saver ips(os);
...
# if defined(BOOST_POSIX_API)
  wjl_boost::int_least64_t tick_factor() // multiplier to convert ticks
                                     //  to nanoseconds; -1 if unknown
  {
    static wjl_boost::int_least64_t tick_factor = 0;
    if (!tick_factor)
    {
...
# endif
  ...
  void get_cpu_times(wjl_boost::timer::cpu_times& current)
  {
    wjl_boost::chrono::duration<wjl_boost::int64_t, wjl_boost::nano>
      x (wjl_boost::chrono::high_resolution_clock::now().time_since_epoch());
...
# if defined(BOOST_WINDOWS_API)
    ...
    {
      current.system = current.user = wjl_boost::timer::nanosecond_type(-1);
    }
# else
    tms tm;
    clock_t c = ::times(&tm);
    ...
    {
      current.system = wjl_boost::timer::nanosecond_type(tm.tms_stime + tm.tms_cstime);
      current.user = wjl_boost::timer::nanosecond_type(tm.tms_utime + tm.tms_cutime);
      wjl_boost::int_least64_t factor;
      ...
      {
        current.user = current.system = wjl_boost::timer::nanosecond_type(-1);
      }
    }
# endif
  }
