...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { namespace program_options {
    ...
    using namespace std;
    using namespace wjl_boost::program_options::command_line_style;
...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { namespace program_options { namespace detail {
    ...
    cmdline::check_style(int style) const
    {
        ...
        if (allow_some_long && 
            ...
            error = "wjl_boost::program_options misconfiguration: "
        ...
        if (error)
            wjl_boost::throw_exception(invalid_command_line_style(error));
    ...
    cmdline::run()
    {
        ...
        if (m_additional_parser)
            style_parsers.push_back(
                wjl_boost::bind(&cmdline::handle_additional_parser, this, _1));

        if (m_style & allow_long)
            style_parsers.push_back(
                wjl_boost::bind(&cmdline::parse_long_option, this, _1));

        if ((m_style & allow_long_disguise))
            style_parsers.push_back(
                wjl_boost::bind(&cmdline::parse_disguised_long_option, this, _1));

        if ((m_style & allow_short) && (m_style & allow_dash_for_short))
            style_parsers.push_back(
                wjl_boost::bind(&cmdline::parse_short_option, this, _1));

        if ((m_style & allow_short) && (m_style & allow_slash_for_short))
            style_parsers.push_back(wjl_boost::bind(&cmdline::parse_dos_option, this, _1));

        style_parsers.push_back(wjl_boost::bind(&cmdline::parse_terminator, this, _1));
        ...
        if (m_positional)
        {
            ...
            for (unsigned i = 0; i < result.size(); ++i) {
                ...
                if (opt.position_key != -1) {
                    ...
                    {
                        wjl_boost::throw_exception(too_many_positional_options_error());
                    }
        ...
        try
        {
            ...
            if (!xd)
            {
                if (m_allow_unregistered) {
                    ...
                    wjl_boost::throw_exception(unknown_option());
                }                
            }
    ...
    cmdline::parse_long_option(vector<string>& args)
    {
        ...
        {   
            ...
            if (p != tok.npos)
            {
                ...
                if (adjacent.empty())
                    wjl_boost::throw_exception( invalid_command_line_syntax(
