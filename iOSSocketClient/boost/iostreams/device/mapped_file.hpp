...
// wjl_boost::filesystem::path, wjl_boost::filesystem::wpath, 
// and wjl_boost::iostreams::detail::path (used to store either a std::string or a
// std::wstring).
    ...
    : detail::mapped_file_params_base 
{
    ...
    // with wjl_boost::filesystem::wpath
...
private:
    ...
    friend struct wjl_boost::iostreams::operations<mapped_file_source>;
public:
...
private:
    ...
    wjl_boost::shared_ptr<impl_type> pimpl_;
};
...
private:
    ...
    typedef basic_mapped_file_params<detail::path>  param_type;
    friend struct wjl_boost::iostreams::operations<mapped_file >;
...
public:
    friend struct wjl_boost::iostreams::operations<mapped_file_sink>;
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, wjl_boost::intmax_t offset)
...
void mapped_file_source::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        if (params.flags != mapped_file::readonly)
            wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid flags"));
    } else {
        if (params.mode & BOOST_IOS::out)
            wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid mode"));
...
mapped_file_sink::mapped_file_sink(
    ...
    wjl_boost::intmax_t offset, mapmode flags )
...
struct operations<mapped_file_source>
    : wjl_boost::iostreams::detail::close_impl<closable_tag>
{
