...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(wjl_boost::declval<T>() = wjl_boost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(wjl_boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(wjl_boost::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
