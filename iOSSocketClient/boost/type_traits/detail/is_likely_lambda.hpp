...
//
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost{ namespace type_traits_detail{
...
{
  template<typename Q> static wjl_boost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static wjl_boost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename wjl_boost::enable_if_c<has_one_operator_call<T>::value>::type> : 
     wjl_boost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public wjl_boost::integral_constant<bool, false> {};
}}
