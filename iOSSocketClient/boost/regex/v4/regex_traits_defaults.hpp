...
inline bool is_separator(charT c)
{
   return BOOST_REGEX_MAKE_BOOL(
      ...
      || (static_cast<wjl_boost::uint16_t>(c) == 0x2028u)
      || (static_cast<wjl_boost::uint16_t>(c) == 0x2029u)
      || (static_cast<wjl_boost::uint16_t>(c) == 0x85u));
}
template <>
...
wjl_boost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   (void)t; // warning suppression
   wjl_boost::intmax_t limit = (std::numeric_limits<wjl_boost::intmax_t>::max)() / radix;
   wjl_boost::intmax_t next_value = t.value(*p1, radix);
   if((p1 == p2) || (next_value < 0) || (next_value >= radix))
      return -1;
   wjl_boost::intmax_t result = 0;
   while(p1 != p2)
   {
