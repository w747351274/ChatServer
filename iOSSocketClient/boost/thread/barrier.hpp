...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost
{
  namespace thread_detail
  {
    ...
    struct void_functor_barrier_reseter
    {
      ...
      : size_(size), fct_(wjl_boost::move(funct))
      {}
      template <typename F>
      ...
      //size_(BOOST_THREAD_RV(other).size_), fct_(wjl_boost::move(BOOST_THREAD_RV(other).fct_))
      {
      }
  ...
  class barrier
  {
    ...
    {
      if (count == 0) wjl_boost::throw_exception(
  ...
  public:
    ...
    bool wait()
    {
      wjl_boost::unique_lock < wjl_boost::mutex > lock(m_mutex);
