...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost
{
    namespace detail
    {
        ...
        public:
            ...
            static bool no_waiters(wjl_boost::intrusive_ptr<basic_cv_list_entry> const& entry)
            {
        ...
        {
            wjl_boost::mutex internal_mutex;
            long total_count;
            unsigned active_generation_count;

            typedef basic_cv_list_entry list_entry;

            typedef wjl_boost::intrusive_ptr<list_entry> entry_ptr;
            ...
            {
                wjl_boost::lock_guard<wjl_boost::mutex> lk(internal_mutex);
                if(!wake_sem)
                {
            ...
            struct entry_manager
            {
                ...
                entry_manager(entry_ptr&& entry_, wjl_boost::mutex& mutex_):
                    entry(static_cast< entry_ptr&& >(entry_)), internal_mutex(mutex_)
                {}
#else
                entry_manager(entry_ptr const& entry_, wjl_boost::mutex& mutex_):
                ...
                {
                  if (entry) {
                    wjl_boost::lock_guard<wjl_boost::mutex> internal_lock(internal_mutex);
                    entry->remove_waiter();
                    entry.reset();
                  }
                }
    ...
    public:
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_wait(unique_lock<mutex>& m,wjl_boost::system_time const& abs_time)
        {
        ...
        bool timed_wait(unique_lock<mutex>& m,wjl_boost::xtime const& abs_time)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,wjl_boost::system_time const& abs_time,predicate_type pred)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,wjl_boost::xtime const& abs_time,predicate_type pred)
        {
    ...
    public:
        ...
        template<typename lock_type>
        bool timed_wait(lock_type& m,wjl_boost::xtime const& abs_time)
        {
            return timed_wait(m, system_time(abs_time));
        }
        ...
        template<typename lock_type,typename predicate_type>
        bool timed_wait(lock_type& m,wjl_boost::xtime const& abs_time,predicate_type pred)
        {
            return timed_wait(m, system_time(abs_time), pred);
        }
