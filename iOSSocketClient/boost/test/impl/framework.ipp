...
private:
    bool            filter_unit( test_unit const& tu )
    {
        ...
        // look for match
        using namespace wjl_boost::placeholders;
        return std::find_if( filters.begin(), filters.end(), bind( &component::pass, _1, wjl_boost::ref(tu) ) ) != filters.end();
    }
...
public:
                                        ...
                                        random_generator_helper const * const p_random_generator = 0)
    {
        ...
        BOOST_TEST_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
            ...
            result = unit_test_monitor.execute_and_translate( wjl_boost::bind( &test_unit_fixture::setup, F ) );
        ...
        if( !unit_test_monitor.is_critical_error( result ) ) {
            ...
            BOOST_TEST_REVERSE_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
                ut_detail::test_unit_id_restore restore_current_test_unit(m_curr_test_unit, tu.p_id);
                result = (std::min)( result, unit_test_monitor.execute_and_translate( wjl_boost::bind( &test_unit_fixture::teardown, F ), 0 ) );
    ...
    wjl_boost::execution_monitor m_aux_em;
...
setup_loggers()
{
    ...
    BOOST_TEST_I_TRY {
        ...
        if( !has_combined_logger ) {
            ...
            if( runtime_config::has( runtime_config::btrt_log_sink ) ) {
                ...
                wjl_boost::function< void () > log_cleaner = wjl_boost::bind( &unit_test_log_t::set_stream,
                                                                      &unit_test_log,
                                                                      wjl_boost::ref(std::cout)
                                                                      );
            ...
            BOOST_TEST_FOREACH( const_string, current_multi_config, v_output_format ) {
                ...
                BOOST_TEST_FOREACH( std::string const&, current_config, v_processed_tokens ) {
                    ...
                    BOOST_TEST_I_ASSRT( format != OF_INVALID,
                                        wjl_boost::runtime::access_to_missing_argument()
    ...
    BOOST_TEST_I_CATCH( wjl_boost::runtime::init_error, ex ) {
        BOOST_TEST_SETUP_ASSERT( false, ex.msg );
    }
    BOOST_TEST_I_CATCH( wjl_boost::runtime::input_error, ex ) {
        std::cerr << ex.msg << "\n\n";

        BOOST_TEST_I_THROW( framework::nothing_to_test( wjl_boost::exit_exception_failure ) );
    }
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    if( runtime_config::has( runtime_config::btrt_report_sink ) ) {
        wjl_boost::function< void () > report_cleaner = wjl_boost::bind( &results_reporter::set_stream,
                                                                 wjl_boost::ref(std::cerr)
                                                                );
    ...
    BOOST_TEST_I_TRY {
        s_frk_state().m_aux_em.vexecute( wjl_boost::bind( &impl::invoke_init_func, init_func ) );
    }
...
int
add_context( ::wjl_boost::unit_test::lazy_ostream const& context_descr, bool sticky )
{
    ...
    if( call_start_finish ) {
        ...
        BOOST_TEST_FOREACH( test_observer*, to, impl::s_frk_state().m_observers ) {
            BOOST_TEST_I_TRY {
                ...
                unit_test_monitor_t::error_level result = unit_test_monitor.execute_and_translate( wjl_boost::bind( &test_observer::test_start, to, tcc.p_count ) );
                if( init_ok ) {
                    if( result != unit_test_monitor_t::test_ok ) {
                        init_ok = false;
                    }
