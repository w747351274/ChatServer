...
/// - exposing an interface to the test framework (as a wjl_boost::unit_test::test_observer)
...
///   @c wjl_boost::unit_test::unit_test_log.
///
/// Users/developers willing to implement their own formatter need to:
/// - implement a wjl_boost::unit_test::unit_test_log_formatter that will output the desired format
/// - register the formatter during a eg. global fixture using the method @c set_formatter (though the framework singleton).
///
/// @warning this observer has a higher priority than the @ref wjl_boost::unit_test::results_collector_t. This means
/// that the various @ref wjl_boost::unit_test::test_results associated to each test unit may not be available at the time
/// the @c test_unit_start, @c test_unit_finish ... are called.
///
/// @see
/// - wjl_boost::unit_test::test_observer
/// - wjl_boost::unit_test::unit_test_log_formatter
...
#define BOOST_TEST_LOG_ENTRY( ll )                                                  \
    (::wjl_boost::unit_test::unit_test_log                                              \
        << ::wjl_boost::unit_test::log::begin( BOOST_TEST_L(__FILE__), __LINE__ ))(ll)  \
/**/
...
#define BOOST_TEST_FRAMEWORK_MESSAGE( M )                       \
   (::wjl_boost::unit_test::unit_test_log                           \
        << ::wjl_boost::unit_test::log::begin(                      \
                "boost.test framework",                         \
                __LINE__ ))                                     \
             ( ::wjl_boost::unit_test::log_messages )               \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/


#define BOOST_TEST_MESSAGE( M )                                 \
    BOOST_TEST_LOG_ENTRY( ::wjl_boost::unit_test::log_messages )    \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/
...
#define BOOST_TEST_PASSPOINT()                                  \
    ::wjl_boost::unit_test::unit_test_log.set_checkpoint(           \
...
#define BOOST_TEST_CHECKPOINT( M )                              \
    ::wjl_boost::unit_test::unit_test_log.set_checkpoint(           \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        (::wjl_boost::wrap_stringstream().ref() << M).str() )       \
/**/
