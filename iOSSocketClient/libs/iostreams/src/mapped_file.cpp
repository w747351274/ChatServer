...
{
    if (is_open())
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("file already open"));
    p.normalize();
    open_file(p);
...
{
    if (!is_open())
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("file is closed"));
...
void mapped_file_impl::open_file(param_type p)
{
    ...
    if (get_size) {
        ...
        if (get_size(handle_, &info)) {
            wjl_boost::intmax_t size =
                ( (static_cast<wjl_boost::intmax_t>(info.HighPart) << 32) |
                  info.LowPart );
            size_ =
                static_cast<std::size_t>(
                    p.length != max_length ?
                        std::min<wjl_boost::intmax_t>(p.length, size) :
                        size
                );
        } else {
        ...
        {
            wjl_boost::intmax_t size =
                (static_cast<wjl_boost::intmax_t>(hi) << 32) | low;
            size_ =
                static_cast<std::size_t>(
                    p.length != max_length ?
                        std::min<wjl_boost::intmax_t>(p.length, size) :
...
#endif
    clear(true);
    wjl_boost::iostreams::detail::throw_system_failure(msg);
}
...
{
    if (mode && flags)
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE(
    ...
    if (offset < 0)
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid offset"));
    if (new_file_size < 0)
        wjl_boost::throw_exception(
