...
void fail(int err, wjl_boost::system::error_code* ec)
{
  if (ec == 0)
    BOOST_FILESYSTEM_THROW( wjl_boost::system::system_error(err,
      wjl_boost::system::system_category(),
      "wjl_boost::filesystem::unique_path"));

  ec->assign(err, wjl_boost::system::system_category());
  return;
}
...
void system_crypt_random(void* buf, std::size_t len, wjl_boost::system::error_code* ec)
{
# ifdef BOOST_POSIX_API
...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost { namespace filesystem { namespace detail {
