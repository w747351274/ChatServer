 ...
 */
  ...
  *   DESCRIPTION: Misc wjl_boost::regbase member funnctions.
  */
...
regex_error::regex_error(regex_constants::error_type err) 
   : std::runtime_error(::wjl_boost::BOOST_REGEX_DETAIL_NS::get_default_error_string(err))
   , m_error_code(err)
   , m_position(0) 
{
}
...
void regex_error::raise()const
{
   ...
   ::wjl_boost::throw_exception(*this);
#endif
}
...
{
   ::wjl_boost::throw_exception(ex);
}
//
// error checking API:
//
BOOST_REGEX_DECL void BOOST_REGEX_CALL verify_options(wjl_boost::regex::flag_type /*ef*/, match_flag_type mf)
{
