...
namespace wjl_boost {} namespace boost = wjl_boost; namespace wjl_boost {
   ...
   namespace detail
   {
      ...
      template <class T>          struct has_nothrow_assign_imp<T, false, true>{ static const bool value = noexcept(wjl_boost::declval<typename add_reference<T>::type>() = wjl_boost::declval<typename add_reference<T const>::type>()); };
