...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::wjl_boost::is_function<typename wjl_boost::remove_reference<T>::type>::value || (sizeof(detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(wjl_boost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : wjl_boost::integral_constant<bool, ::wjl_boost::is_function<typename wjl_boost::remove_reference<T>::type>::value || ::wjl_boost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : wjl_boost::is_complete<T> {};
   
#else

      template <class T> struct is_complete
         : public wjl_boost::integral_constant<bool, true> {};
