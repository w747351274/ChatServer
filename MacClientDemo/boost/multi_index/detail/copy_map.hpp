...
public:
  ...
  ~copy_map()
  {
    if(!released){
      for(std::size_t i=0;i<n;++i){
        wjl_boost::detail::allocator::destroy(
          wjl_boost::addressof((spc.data()+i)->second->value()));
  ...
  void clone(Node* node)
  {
    ...
    BOOST_TRY{
      wjl_boost::detail::allocator::construct(
        wjl_boost::addressof((spc.data()+n)->second->value()),node->value());
    }
    BOOST_CATCH(...){
...
private:
  typedef typename wjl_boost::detail::allocator::rebind_to<
    Allocator,Node
  >::type                                               allocator_type;
