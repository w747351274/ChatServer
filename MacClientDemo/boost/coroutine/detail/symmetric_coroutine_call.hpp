...
# ifdef BOOST_MSVC
        ...
        impl_( 0)
    {
        ...
        impl_ = new ( sp) object_t(
                    wjl_boost::forward< coroutine_fn >( fn), attrs, preallocated( sp, size, stack_ctx), stack_alloc); 
        BOOST_ASSERT( impl_);
    }
...
# endif
        ...
        impl_( 0)
    {
        ...
        // placement new for internal coroutine
        impl_ = new ( sp) object_t(
                    wjl_boost::forward< Fn >( fn), attrs, preallocated( sp, size, stack_ctx), stack_alloc); 
        BOOST_ASSERT( impl_);
    }
    ...
    {
        symmetric_coroutine_call tmp( wjl_boost::move( other) );
        swap( tmp);
        return * this;
    }
