...
template <typename Clock,
    typename WaitTraits = wjl_boost::asio::wait_traits<Clock>
 ...
 *
 * Most applications will use one of the wjl_boost::asio::steady_timer,
 * wjl_boost::asio::system_timer or wjl_boost::asio::high_resolution_timer typedefs.
 *
 ...
 * wjl_boost::asio::steady_timer timer(io_context);
 *
 ...
 * @code
 * void handler(const wjl_boost::system::error_code& error)
 * {
 *   if (!error)
 *   {
 *     // Timer expired.
 ...
 * }
 *
 * void on_timeout(const wjl_boost::system::error_code& e)
 * {
 *   if (e != wjl_boost::asio::error::operation_aborted)
 *   {
 ...
 * @endcode
 *
 * @li The wjl_boost::asio::basic_waitable_timer::expires_after() function
 * cancels any pending asynchronous waits, and returns the number of
 * asynchronous waits that were cancelled. If it returns 0 then you were too
 * late and the wait handler has already been executed, or will soon be
 * executed. If it returns 1 then the wait handler was successfully cancelled.
 *
 * @li If a wait handler is cancelled, the wjl_boost::system::error_code passed to
 * it contains the value wjl_boost::asio::error::operation_aborted.
 */
...
public:
   ...
   */
  explicit basic_waitable_timer(wjl_boost::asio::io_context& io_context)
   ...
   * as an absolute time.
   */
  basic_waitable_timer(wjl_boost::asio::io_context& io_context,
      const time_point& expiry_time)
    : basic_io_object<BOOST_ASIO_SVC_T>(io_context)
  {
    wjl_boost::system::error_code ec;
    this->get_service().expires_at(this->get_implementation(), expiry_time, ec);
    wjl_boost::asio::detail::throw_error(ec, "expires_at");
  }
    ...
    : basic_io_object<BOOST_ASIO_SVC_T>(io_context)
  {
    ...
    wjl_boost::asio::detail::throw_error(ec, "expires_after");
  }
   ...
   * be invoked with the wjl_boost::asio::error::operation_aborted error code.
   *
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel(wjl_boost::system::error_code& ec)
  {
   ...
   * wjl_boost::asio::error::operation_aborted error code.
   *
   * Cancelling the timer does not change the expiry time.
   *
  ...
  std::size_t cancel_one()
  {
    ...
    wjl_boost::asio::detail::throw_error(ec, "cancel_one");
    return s;
  }
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel_one(wjl_boost::system::error_code& ec)
  {
  ...
  std::size_t expires_from_now(const duration& expiry_time)
  {
    ...
    std::size_t s = this->get_service().expires_from_now(
        this->get_implementation(), expiry_time, ec);
    wjl_boost::asio::detail::throw_error(ec, "expires_from_now");
    return s;
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  void wait(wjl_boost::system::error_code& ec)
  {
    this->get_service().wait(this->get_implementation(), ec);
  }
   ...
   * code wjl_boost::asio::error::operation_aborted.
   *
