...
struct prepared_buffers_max<wjl_boost::array<Elem, N> >
{
  enum { value = N };
};
...
public:
  ...
  explicit consuming_buffers(const Buffers& buffers)
    : buffers_(buffers),
      ...
      next_elem_offset_(0)
  {
    using wjl_boost::asio::buffer_size;
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = wjl_boost::asio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = wjl_boost::asio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = wjl_boost::asio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return wjl_boost::asio::buffer(buffer_ + total_consumed_, max_size);
  }
...
class consuming_buffers<Buffer, wjl_boost::array<Elem, 2>,
    typename wjl_boost::array<Elem, 2>::const_iterator>
{
public:
  // Construct to represent the entire list of buffers.
  explicit consuming_buffers(const wjl_boost::array<Elem, 2>& buffers)
    : buffers_(buffers),
      total_consumed_(0)
  {
  }
  ...
  // Get the buffer for a single transfer, with a size.
  wjl_boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    wjl_boost::array<Buffer, 2> result = {{
      Buffer(buffers_[0]), Buffer(buffers_[1]) }};
    std::size_t buffer0_size = result[0].size();
    result[0] = wjl_boost::asio::buffer(result[0] + total_consumed_, max_size);
    result[1] = wjl_boost::asio::buffer(
...
private:
  wjl_boost::array<Elem, 2> buffers_;
  std::size_t total_consumed_;
};
...
class consuming_buffers<Buffer, null_buffers, const mutable_buffer*>
  : public wjl_boost::asio::null_buffers
{
public:
