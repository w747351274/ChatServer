...
wjl_boost::system::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
...
wjl_boost::system::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    wjl_boost::system::error_code& ec)
{
  if (!is_open(impl))
...
wjl_boost::system::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, wjl_boost::system::error_code& ec)
{
  if (is_open(impl))
  {
...
wjl_boost::system::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, wjl_boost::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = wjl_boost::asio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, wjl_boost::asio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, wjl_boost::asio::error::already_open);
  else
  {
    wjl_boost::system::error_code ec;
    ...
    std::size_t addrlen, win_iocp_socket_connect_op_base* op)
{
      ...
      || family == BOOST_ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      if (op->ec_ && op->ec_ != wjl_boost::asio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == wjl_boost::asio::error::in_progress
          || op->ec_ == wjl_boost::asio::error::would_block)
      {
