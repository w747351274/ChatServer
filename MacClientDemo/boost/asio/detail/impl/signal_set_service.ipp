...
signal_set_service::signal_set_service(
  ...
  : service_base<signal_set_service>(io_context),
    io_context_(wjl_boost::asio::use_service<io_context_impl>(io_context)),
#if !defined(BOOST_ASIO_WINDOWS) \
  && !defined(BOOST_ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
    reactor_(wjl_boost::asio::use_service<reactor>(io_context)),
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
    next_(0),
    ...
    wjl_boost::asio::io_context::fork_event fork_ev)
{
  ...
  switch (fork_ev)
  {
  case wjl_boost::asio::io_context::fork_prepare:
    {
    ...
    break;
  case wjl_boost::asio::io_context::fork_parent:
    ...
    break;
  case wjl_boost::asio::io_context::fork_child:
    if (state->fork_prepared_)
    {
      wjl_boost::asio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
wjl_boost::system::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, wjl_boost::system::error_code& ec)
{
...
wjl_boost::system::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, wjl_boost::system::error_code& ec)
{
  // Check that the signal number is valid.
...
wjl_boost::system::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
wjl_boost::system::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    wjl_boost::system::error_code& ec)
{
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = wjl_boost::asio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    wjl_boost::system::error_code ec(errno,
        wjl_boost::asio::error::get_system_category());
    wjl_boost::asio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
