...
namespace wjl_boost_asio_handler_alloc_helpers {
...
#else
  using wjl_boost::asio::asio_handler_allocate;
  return asio_handler_allocate(s, wjl_boost::asio::detail::addressof(h));
#endif
}
...
inline void deallocate(void* p, std::size_t s, Handler& h)
{
  ...
  ::operator delete(p);
#else
  using wjl_boost::asio::asio_handler_deallocate;
  asio_handler_deallocate(p, s, wjl_boost::asio::detail::addressof(h));
#endif
}
...
public:
  ...
  T* allocate(std::size_t n)
  {
    return static_cast<T*>(
        wjl_boost_asio_handler_alloc_helpers::allocate(sizeof(T) * n, handler_));
  }

  void deallocate(T* p, std::size_t n)
  {
    wjl_boost_asio_handler_alloc_helpers::deallocate(p, sizeof(T) * n, handler_);
  }
...
#define BOOST_ASIO_DEFINE_HANDLER_PTR(op) \
  ...
  { \
    ...
    { \
      typedef typename ::wjl_boost::asio::associated_allocator< \
        Handler>::type associated_allocator_type; \
      typedef typename ::wjl_boost::asio::detail::get_hook_allocator< \
        Handler, associated_allocator_type>::type hook_allocator_type; \
      BOOST_ASIO_REBIND_ALLOC(hook_allocator_type, op) a( \
            ::wjl_boost::asio::detail::get_hook_allocator< \
              Handler, associated_allocator_type>::get( \
                handler, ::wjl_boost::asio::get_associated_allocator(handler))); \
      return a.allocate(1); \
    } \
    void reset() \
    { \
      ...
      { \
        ...
        BOOST_ASIO_REBIND_ALLOC(hook_allocator_type, op) a( \
              ::wjl_boost::asio::detail::get_hook_allocator< \
                Handler, associated_allocator_type>::get( \
                  *h, ::wjl_boost::asio::get_associated_allocator(*h))); \
...
#define BOOST_ASIO_DEFINE_HANDLER_ALLOCATOR_PTR(op) \
  ...
  { \
    ...
    { \
      typedef typename ::wjl_boost::asio::detail::get_recycling_allocator< \
        Alloc>::type recycling_allocator_type; \
      BOOST_ASIO_REBIND_ALLOC(recycling_allocator_type, op) a1( \
            ::wjl_boost::asio::detail::get_recycling_allocator<Alloc>::get(a)); \
