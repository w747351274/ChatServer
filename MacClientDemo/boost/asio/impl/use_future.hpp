    ...
    if (ec)
    {
      this->p_->set_exception(
          std::make_exception_ptr(
            wjl_boost::system::system_error(ec)));
    }
    else
    {
      this->p_->set_value();
...
template <>
class promise_handler_selector<void(wjl_boost::system::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(wjl_boost::system::error_code, Arg)>
...
#define BOOST_ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  template <typename Arg, BOOST_ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(wjl_boost::system::error_code, Arg, BOOST_ASIO_VARIADIC_TARGS(n))> \
