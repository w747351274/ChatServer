...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(wjl_boost::system::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const wjl_boost::system::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = wjl_boost::asio::buffer_copy(
    ...
    const ConstBufferSequence& buffers)
{
  ...
  storage_.resize(orig_size + length);
  return wjl_boost::asio::buffer_copy(
