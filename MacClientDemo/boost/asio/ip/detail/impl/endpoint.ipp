  ...
  : data_()
{
  ...
  if (family == BOOST_ASIO_OS_DEF(AF_INET))
  {
    data_.v4.sin_family = BOOST_ASIO_OS_DEF(AF_INET);
    data_.v4.sin_port =
      wjl_boost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v4.sin_addr.s_addr = BOOST_ASIO_OS_DEF(INADDR_ANY);
  }
  else
  {
...
endpoint::endpoint(const wjl_boost::asio::ip::address& addr,
  ...
  : data_()
{
  ...
  if (addr.is_v4())
  {
    ...
    data_.v4.sin_addr.s_addr =
      wjl_boost::asio::detail::socket_ops::host_to_network_long(
        addr.to_v4().to_uint());
  }
  else
  {
    ...
    data_.v6.sin6_port =
      wjl_boost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v6.sin6_flowinfo = 0;
    wjl_boost::asio::ip::address_v6 v6_addr = addr.to_v6();
    wjl_boost::asio::ip::address_v6::bytes_type bytes = v6_addr.to_bytes();
    memcpy(data_.v6.sin6_addr.s6_addr, bytes.data(), 16);
    data_.v6.sin6_scope_id =
      static_cast<wjl_boost::asio::detail::u_long_type>(
        v6_addr.scope_id());
  }
}
...
{
  if (new_size > sizeof(wjl_boost::asio::detail::sockaddr_storage_type))
  {
    wjl_boost::system::error_code ec(wjl_boost::asio::error::invalid_argument);
...
unsigned short endpoint::port() const
{
  if (is_v4())
  {
    return wjl_boost::asio::detail::socket_ops::network_to_host_short(
        data_.v4.sin_port);
  }
  else
  {
...
wjl_boost::asio::ip::address endpoint::address() const
{
  using namespace std; // For memcpy.
  if (is_v4())
  {
    return wjl_boost::asio::ip::address_v4(
        wjl_boost::asio::detail::socket_ops::network_to_host_long(
          data_.v4.sin_addr.s_addr));
  }
  else
  {
    wjl_boost::asio::ip::address_v6::bytes_type bytes;
#if defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.data(), data_.v6.sin6_addr.s6_addr, 16);
#else // defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.elems, data_.v6.sin6_addr.s6_addr, 16);
#endif // defined(BOOST_ASIO_HAS_STD_ARRAY)
    return wjl_boost::asio::ip::address_v6(bytes, data_.v6.sin6_scope_id);
  }
}

void endpoint::address(const wjl_boost::asio::ip::address& addr)
{
