...
public:
    void    operator()()    { TestCaseTemplate::run( (wjl_boost::type<TestType>*)0 ); }
};
...
struct generate_test_case_4_type {
    ...
    void        operator()( mpl::identity<TestType> )
    {
        ...
        full_name += '<';
#if !defined(BOOST_NO_TYPEID) && !defined(BOOST_NO_RTTI)
        full_name += wjl_boost::core::demangle(typeid(TestType).name()); // same as execution_monitor.ipp
#else
        full_name += BOOST_CURRENT_FUNCTION;
#endif
        if( wjl_boost::is_const<TestType>::value )
            full_name += "_const";
        full_name += '>';
