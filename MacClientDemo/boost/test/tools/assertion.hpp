...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(wjl_boost::declval<Lhs>() oper wjl_boost::declval<Rhs>() ) optype; \
    typedef typename wjl_boost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _ )                         \
    template<typename T>                                        \
    binary_expr<ExprType,typename wjl_boost::decay<T const>::type,  \
        op::name<ValType,typename wjl_boost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename wjl_boost::decay<T const>::type Rhs;       \
