...
public:
    ...
    void            setup( const const_string& stream_name,
                           wjl_boost::function<void ()> const &cleaner_callback = wjl_boost::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = wjl_boost::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(wjl_boost::function<void ()> cleaner_callback)
        ...
        wjl_boost::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    wjl_boost::shared_ptr<callback_cleaner>   m_cleaner;
