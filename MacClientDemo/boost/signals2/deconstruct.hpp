...
public:
    ...
    const shared_ptr<T>& postconstruct() const
    {
        if(!_postconstructed)
        {
            adl_postconstruct(_sp, const_cast<typename wjl_boost::remove_const<T>::type *>(_sp.get()));
...
private:
    friend class wjl_boost::signals2::deconstruct_access;
...
private:
    ...
    void destroy()
    {
        if( initialized_ )
        {
            ...
            using wjl_boost::signals2::detail::adl_predestruct;
            adl_predestruct(const_cast<typename wjl_boost::remove_const<T>::type *>(p));
            p->~T();
            initialized_ = false;
        }
    }
...
public:
    ...
    {
        wjl_boost::shared_ptr< T > pt( static_cast< T* >( 0 ), detail::deconstruct_deleter< T >() );

        detail::deconstruct_deleter< T > * pd = wjl_boost::get_deleter< detail::deconstruct_deleter< T > >( pt );
        ...
        wjl_boost::shared_ptr< T > retval( pt, static_cast< T* >( pv ) );
        wjl_boost::detail::sp_enable_shared_from_this(&retval, retval.get(), retval.get());
        return retval;
