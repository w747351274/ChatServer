...
void file_descriptor_impl::open(const detail::path& p, BOOST_IOS::openmode mode)
{
    ...
    if ( !(mode & (BOOST_IOS::in | BOOST_IOS::out | BOOST_IOS::app)) ||
        ...
        wjl_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("bad open mode"));
    }
    ...
    if (fd == -1) {
        wjl_boost::throw_exception(system_failure("failed opening file"));
    } else {
    ...
    (stream_offset off, BOOST_IOS::seekdir way)
{
    ...
    {
        wjl_boost::throw_exception(system_failure("failed seeking"));
    } else {
